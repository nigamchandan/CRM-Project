...
exports.register = async (req, res, next) => {
  try {
    ...
    await writeLog({ userId: result.user.id, action: 'auth.register', entity: 'users', entityId: result.user.id });
...
exports.login = async (req, res, next) => {
  try {
    ...
    await writeLog({ userId: result.user.id, action: 'auth.login', entity: 'users', entityId: result.user.id });
    res.json(result);
  } catch (err) { next(err); }
};
...
exports.logout = async (req, res) => {
  await writeLog({ userId: req.user.id, action: 'auth.logout' });
