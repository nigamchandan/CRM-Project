...
exports.create = async (req, res, next) => {
  try {
    ...
    await writeLog({ req, userId: req.user.id, action: 'task.create', entity: 'tasks', entityId: r.id });
...
exports.update = async (req, res, next) => {
  try {
    ...
    await writeLog({ req, userId: req.user.id, action: 'task.update', entity: 'tasks', entityId: r.id });
...
exports.complete = async (req, res, next) => {
  try {
    ...
    await writeLog({ req, userId: req.user.id, action: 'task.complete', entity: 'tasks', entityId: r.id, meta: { completed: r.status === 'completed' } });
...
exports.remove = async (req, res, next) => {
  try {
    ...
    await writeLog({ req, userId: req.user.id, action: 'task.delete', entity: 'tasks', entityId: req.params.id });
