...
function pickUserFields(u) {
  ...
  return {
    ...
    is_active: u.is_active, team_id: u.team_id,
  };
}
...
exports.create = async (req, res, next) => {
  try {
    ...
    await writeLog({ req, userId: req.user.id, action: 'user.create', entity: 'users', entityId: u.id });
...
exports.update = async (req, res, next) => {
  try {
    ...
    await writeLog({
      ...
      entity:   'users',
      entityId: u.id,
      before:   pickUserFields(before),
      after:    pickUserFields(u),
    });
    res.json(u);
  } catch (e) { next(e); }
};
exports.toggleStatus = async (req, res, next) => {
  try {
    ...
    await writeLog({ req, userId: req.user.id, action: 'user.toggle_status', entity: 'users', entityId: u.id, meta: { is_active: u.is_active } });
    res.json(u);
  } catch (e) { next(e); }
};
exports.remove = async (req, res, next) => {
  try {
    ...
    await writeLog({ req, userId: req.user.id, action: 'user.delete', entity: 'users', entityId: req.params.id });
    res.status(204).end();
  } catch (e) { next(e); }
};
